...
import { AREA_COLORS, CATEGORY_EMOJI, quadrant, stripEmoji } from '../utils/feedback';

type TicketNode = Node<SOVERNNodeData>;

const QUADRANTS = [
  // [колонка (urgency-половина), ряд (impact-половина)] в grid-координатах
  { label: 'Schedule', sub: 'high impact · low urgency', color: '#3b82f6', col: 1, row: 1 },
  { label: 'Do First', sub: 'high impact · high urgency', color: '#ef4444', col: 2, row: 1 },
  { label: 'Backlog', sub: 'low impact · low urgency', color: '#475569', col: 1, row: 2 },
  { label: 'Quick Wins', sub: 'low impact · high urgency', color: '#eab308', col: 2, row: 2 },
];
...
function Chip({ node }: { node: TicketNode }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1 max-w-full rounded-md border px-1.5 py-0.5 text-left bg-slate-900/90 backdrop-blur transition-all hover:translate-y-[-1px] hover:shadow-lg hover:shadow-black/40 ${
        selected ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-700/70 hover:border-slate-500'
      }`}
    ...
    >
      ...
      <span
        ...
        style={{ backgroundColor: AREA_COLORS[area] || '#94a3b8' }}
      />
    </button>
  );
}
  ...
  return (
    <div className="absolute inset-0 flex flex-col bg-[#020617] z-10 pt-44 pb-8 px-10">
      <div className="relative flex-1 min-h-0">
        {/* квадрантные фоны */}
        <div className="absolute inset-0 grid grid-cols-2 grid-rows-2 gap-px rounded-2xl overflow-hidden border border-slate-800/60">
          {QUADRANTS.map((q) => (
            <div
              key={q.label}
              className="relative"
              style={{ gridColumn: q.col, gridRow: q.row, backgroundColor: q.color + '08' }}
            >
              <div className="absolute top-3 left-4">
                <div className="text-[13px] font-black uppercase tracking-[0.25em]" style={{ color: q.color + 'cc' }}>{q.label}</div>
                <div className="text-[9px] font-bold uppercase tracking-widest text-slate-600 mt-0.5">{q.sub}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* оси */}
        <div className="absolute -bottom-6 inset-x-0 flex justify-between text-[9px] font-black uppercase tracking-[0.3em] text-slate-600 px-2">
          <span>← low urgency</span><span>high urgency →</span>
        </div>
        <div className="absolute -left-7 inset-y-0 flex flex-col justify-between items-center text-[9px] font-black uppercase tracking-[0.3em] text-slate-600 py-2" style={{ writingMode: 'vertical-rl' }}>
        ...
        {[...cells.entries()].map(([key, arr]) => {
          ...
          return (
            <div
              key={key}
              className={`absolute flex flex-col gap-1 ${isOpen ? 'z-20 max-h-[60vh] overflow-y-auto custom-scrollbar rounded-xl bg-slate-950/90 p-1.5 border border-slate-700/60' : ''}`}
